...
const MapView = () => {
    ...
    const handleFilesChange = (event) => {
        ...
        // 过滤掉无法正常读取的文件
        const validFiles = Array.from(files).filter(file => file.type.startsWith('image/') && file.size > 0);
            ...
            reader.onload = async (e) => {
                ...
                try {
                    ...
                    if (lat && lon) {
                        ...
                        // 转换为 GCJ-02 坐标
                        const { lat: gcjLat, lon: gcjLon } = wgs84ToGcj02(latDecimal, lonDecimal);
    ...
    return (
        <div>
            <div style={styles.fileInputContainer}>
                <input type="file" accept="image/*" multiple onChange={handleFilesChange} style={styles.fileInput} />
